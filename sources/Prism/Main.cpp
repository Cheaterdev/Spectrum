...
// Prism can't include that C++ module, so keep these two lists in sync by hand -
...
// user, moved to a declared field plus Graph::override_resource (ui.prism). Kept
...
static void render_expr(const Parsed& parsed, have_expr& e)
{
	...
	for (const auto& t : e.terms)
	{
		...
		case ExprTerm::Qualified:
			// Owner::name is a context field only if Owner is a Prism-declared
...
// it. Runs after parsed.setup() so parsed.tables is complete across all .prism
...
// that hold regardless of which .prism file declares the item; a per-file
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		iterate_files("defs/", [&](std::wstring filename)
		{
			std::wcout << ((filename + L"\n")) << std::endl;
			auto p = parse(filename);

			// Stamp every top-level named item with its source .prism path so
			// templates can emit an autogenerated-from comment in the output.
			std::string sig_path = "sources/Prism/"
		...
		// .prism renumbers those enums. Loading a plan built against a different ID
		...
		global.AddGlobal("resolve_size_expr", jinja2::MakeCallable(
			[&](const std::string& pass_name, const std::string& field_name) -> std::string
			{
				...
				for (const auto& p : pass->params)
				{
					...
					// Raw (backtick) text is pasted verbatim -- the .prism author
