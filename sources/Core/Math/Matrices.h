...
public:
	...
	static 	matrix rotationZ(float angle) requires(matrix_type::N == 5 && matrix_type::M == 4)
	{
		matrix r;
		float cosA = cos(angle);
		float sinA = sin(angle);
...
Vector<vector_data_t<typename MatrixFormat::Format, MatrixFormat::M>>operator*(const Vector<VectorFormatIn>& v, const  matrix<MatrixFormat>& m) requires(VectorFormatIn::N == MatrixFormat::M)
{
