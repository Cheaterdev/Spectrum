			...
			if (updatedGlyphCount > 0)
			{
				...
				if (dirtyRect.right > dirtyRect.left && dirtyRect.bottom > dirtyRect.top)
				{
					...
					for (UINT i = 0; i < m_mipLevelCount; ++i)
					{
						...
						pContext->get_copy().update_texture(m_pTexture->resource, ivec3(dirtyRect.left, dirtyRect.top, 0), ivec3(dirtyRect.right - dirtyRect.left, dirtyRect.bottom - dirtyRect.top, 1),
