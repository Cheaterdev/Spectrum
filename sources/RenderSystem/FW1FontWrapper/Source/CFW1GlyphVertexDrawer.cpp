...
//        m_pVertexBuffer.reset(new HAL::Buffer<FW1_GLYPHVERTEX>(HAL::HeapType::DEFAULT, m_vertexBufferSize / sizeof(FW1_GLYPHVERTEX)));
		...
		m_pIndexBuffer = Helpers::make_buffer<unsigned int>(HAL::Device::get(), indices);
		return S_OK;
	}
	...
	)
	{
		...
		while (currentVertex < vertexData->TotalVertexCount)
		{
			...
			auto view = data.resource->create_view<HAL::StructuredBufferView<Table::Glyph>>(*pContext, HAL::StructuredBufferViewDesc{ (uint)data.resource_offset, (uint)data.size, HAL::counterType::NONE });
