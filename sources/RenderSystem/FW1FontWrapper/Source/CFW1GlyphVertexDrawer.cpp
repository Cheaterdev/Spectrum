...
//        m_pVertexBuffer.reset(new HAL::Buffer<FW1_GLYPHVERTEX>(HAL::HeapType::DEFAULT, m_vertexBufferSize / sizeof(FW1_GLYPHVERTEX)));
	...
	)
	{
		...
		while (currentVertex < vertexData->TotalVertexCount)
		{
			...
			auto view = data.resource->create_view<HAL::StructuredBufferView<Table::Glyph>>(*pContext->frame_resources, HAL::StructuredBufferViewDesc{ (uint)data.resource_offset, (uint)data.size, false });
