...
void GUI::Elements::FlowGraph::canvas::draw(Graphics::context& c)
{
	...
	if (linking.size())
	{
		...
		auto view = data.resource->create_view<HAL::StructuredBufferView<Table::VSLine>>(*c.command_list->frame_resources, StructuredBufferViewDesc{ (UINT)data.offset, (UINT)data.size,false });
		{
