        ...
        namespace Debug
        {
            ...
            OutputWindow::OutputWindow()
            {
                ...
                Log::get().on_log.register_handler(this, [this](const LogBlock & block)
                {
                    ...
                    if (block.get_level_internal() == static_cast<int>(log_level_internal::level_error))
                        item->color = rgba8(200, 0, 0, 255);

                    if (block.get_level_internal() == static_cast<int>(log_level_internal::level_debug))
                        item->color = rgba8(0, 100, 0, 255);

                    if (block.get_level_internal() == static_cast<int>(log_level_internal::level_warning))
                        item->color = rgba8(150, 150, 0, 255);

                    if (block.get_level_internal() == static_cast<int>(log_level_internal::level_none))
                        item->color = rgba8(200, 200, 200, 255);

                    if (block.get_level_internal() == static_cast<int>(log_level_internal::level_info))
