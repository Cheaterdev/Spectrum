...
public:
	struct Context
	{
		...
		static void load_from_cache([[maybe_unused]] Context& data, [[maybe_unused]] const Cache& cache, [[maybe_unused]] const FrameGraph::TaskBuilder& builder)
		{
			...
			builder.create_versioned(data.SMAA_edges, cache.SMAA_edges, { ivec3(builder.graph->get_context<Table::ViewportContext>().frame_size, 0), HAL::Format::R8G8_UNORM, 1, 1 });
			builder.create_versioned(data.SMAA_blend, cache.SMAA_blend, { ivec3(builder.graph->get_context<Table::ViewportContext>().frame_size, 0), HAL::Format::R8G8B8A8_UNORM, 1, 1 });
		}
