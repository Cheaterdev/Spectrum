		...
		public:
			struct Context
			{
				...
				static void need_always(Context& data, FrameGraph::TaskBuilder& builder)
				{
					...
					if (builder.graph->get_context<Table::Shadows::VSM::VSMSelectors>().shadow_source == ::Shadows::VSM::ShadowSource::RTXReference && builder.exists(data.VSM_ShadowNoise))
						builder.need(data.VSM_ShadowNoise, FrameGraph::ResourceFlags::Read);
					if (builder.graph->get_context<Table::Shadows::VSM::VSMSelectors>().shadow_source == ::Shadows::VSM::ShadowSource::VSM && builder.exists(data.VSM_PCSS_ShadowNoise))
				...
				{
					builder.create(data.VSM_ShadowDenoised, { ivec3(builder.graph->get_context<Table::Frame::ViewportContext>().frame_size, 0), HAL::Format::R8_UNORM, 1, 1 }, FrameGraph::ResourceFlags::UnorderedAccess);
				}
				...
				static void load_from_cache([[maybe_unused]] Context& data, [[maybe_unused]] const Cache& cache, [[maybe_unused]] const FrameGraph::TaskBuilder& builder)
				{
					...
					builder.create_versioned(data.VSM_ShadowDenoised, cache.VSM_ShadowDenoised, { ivec3(builder.graph->get_context<Table::Frame::ViewportContext>().frame_size, 0), HAL::Format::R8_UNORM, 1, 1 });
				}
