...
public:
	struct Context
	{
		...
		static void load_from_cache([[maybe_unused]] Context& data, [[maybe_unused]] const Cache& cache, [[maybe_unused]] const FrameGraph::TaskBuilder& builder)
		{
			...
			builder.create_versioned(data.RTXReflectionNoise, cache.RTXReflectionNoise, { ivec3(builder.graph->get_context<Table::ViewportContext>().frame_size, 0), HAL::Format::R16G16B16A16_FLOAT, 1, 1 });
			builder.create_versioned(data.RTXReflectionDirPdf, cache.RTXReflectionDirPdf, { ivec3(builder.graph->get_context<Table::ViewportContext>().frame_size, 0), HAL::Format::R16G16B16A16_FLOAT, 1, 1 });
		}
