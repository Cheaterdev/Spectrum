...
The VSIX also carries the language server: SigLanguageClient.cs (compiled here
with the csc and VS assemblies from the local install) starts the bundled
bin/profile/sigparser.exe with --lsp, which reports the same errors a generator
...
<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
  <Metadata>
    ...
    <Description xml:space="preserve">Syntax highlighting and live diagnostics for Spectrum .sig files.</Description>
  </Metadata>
  <Installation>
  ...
  <Assets>
    ...
    <Asset Type="Microsoft.VisualStudio.MefComponent" Path="SigLanguageClient.dll" />
  </Assets>
  <Prerequisites>
...
def main():
    ...
    # The server is whatever sigparser.exe was last built, so rebuild SIGParser
    # (Profile) before regenerating when its validation changed.
    if not os.path.exists(SERVER_EXE):
        sys.exit(f"missing {SERVER_EXE}: build the sigparser project (Profile) first")
    binaries = {}
    with tempfile.TemporaryDirectory() as tmp:
        with open(compile_client(tmp), "rb") as f:
            binaries["SigLanguageClient.dll"] = f.read()
