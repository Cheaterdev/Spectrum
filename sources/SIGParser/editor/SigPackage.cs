// Tools > Regenerate SIG code (also on the SIG toolbar). Compiled into
// SigLanguageClient.dll by gen_vs_extension.py, which also writes this
// package's registration into sig.pkgdef -- there is no RegPkg step.
using System;
...
{
    [Guid(PackageGuid)]
    public sealed class SigPackage : AsyncPackage
    {
        // Must match guidSigPackage / guidSigCmdSet in SigCommands.vsct.
        public const string PackageGuid = "ffdc10aa-ef2c-4958-b04a-0472de298aaa";
        static readonly Guid CommandSet = new Guid("922623a0-3f03-48f5-baa7-9e37559a407f");
        const int RegenerateId = 0x0100;

        static readonly Guid PaneGuid = new Guid("5d0e5b1e-7c1a-4f6b-9f5e-2a3c9d1b7e41");
        ...
        protected override async Task InitializeAsync(CancellationToken token, IProgress<ServiceProgressData> progress)
        {
            ...
            var command = new OleMenuCommand((s, e) => JoinableTaskFactory.RunAsync(RegenerateAsync).FileAndForget("sig/regenerate"),
        ...
        async Task RegenerateAsync()
        {
            ...
            // The generator reads .sig files from disk.
            foreach (Document doc in dte.Documents)
                if (!doc.Saved && doc.FullName.EndsWith(".sig", StringComparison.OrdinalIgnoreCase))
                    doc.Save();

            string repo = FindRepo(dte);
            if (repo == null)
            {
                pane.OutputStringThreadSafe("Could not find sources\\SIGParser\\sigs above the solution or the active document.\n");
                ShowMessage("Could not find the Spectrum checkout (sources\\SIGParser\\sigs) for this solution.", OLEMSGICON.OLEMSGICON_WARNING);
                return;
            }

            string sigDir = Path.Combine(repo, "sources", "SIGParser");
            string exe = PickGenerator(repo);
            pane.OutputStringThreadSafe($"Generator: {exe}\nWorking directory: {sigDir}\n\n");
            SetStatus("Regenerating SIG code...");
            ...
            catch (Exception e)
            {
                ...
                SetStatus("SIG generation could not start.");
                return;
            }
            finally
            {
            ...
            if (exitCode != 0)
            {
                ...
                string what = errors > 0 ? $"{errors} error(s) in .sig files" : $"exit code {exitCode}";
                SetStatus($"SIG generation failed: {what}; nothing was written.");
                ShowMessage($"SIG generation failed ({what}); nothing was written.\n\nDetails are in the Output window, \"SIG\" pane.",
            ...
            SetStatus($"SIG regenerated: {modified.Count} modified, {added.Count} added, {removed.Count} removed.");
        ...
        static string FindRepo(DTE2 dte)
        {
            ...
            foreach (string start in starts)
                for (var dir = new DirectoryInfo(start); dir != null; dir = dir.Parent)
                    if (Directory.Exists(Path.Combine(dir.FullName, "sources", "SIGParser", "sigs")))
                        return dir.FullName;
            return null;
        }
        ...
        {
            string bundled = Path.Combine(Path.GetDirectoryName(typeof(SigPackage).Assembly.Location), "server", "sigparser.exe");
            string local = Path.Combine(repo, "bin", "profile", "sigparser.exe");
        ...
        IVsOutputWindowPane GetPane()
        {
            ...
            {
                window.CreatePane(ref guid, "SIG", 1, 1);
        ...
        {
            return VsShellUtilities.ShowMessageBox(this, text, "SIG", icon, buttons, OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST);
        }
    }
}
