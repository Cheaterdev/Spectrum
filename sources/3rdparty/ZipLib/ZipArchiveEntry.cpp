...
std::istream* ZipArchiveEntry::GetDecompressionStream(std::istream& stream)
{
    ...
    {
        auto offsetOfCompressedData = this->GetOffsetOfCompressedData();
        ...
        stream.seekg(this->GetOffsetOfCompressedData(), std::ios::beg);
        // make correctly-ended substream of the input stream
        intermediateStream = _archiveStream = std::make_shared<isubstream>(stream, offsetOfCompressedData, this->GetCompressedSize());
