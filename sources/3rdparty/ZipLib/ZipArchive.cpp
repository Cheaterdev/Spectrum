...
ZipArchive::Ptr ZipArchive::Create(ZipArchive::Ptr&& other)
{
  ...
  result->owned_stream = other->owned_stream;
...
ZipArchive::Ptr ZipArchive::Create(std::shared_ptr<std::istream> stream)
{
	ZipArchive::Ptr result(new ZipArchive());
	result->owned_stream = stream;

	result->_zipStream = stream.get();
	result->_owningStream = false;
...
ZipArchive& ZipArchive::operator = (ZipArchive&& other)
{
  ...
  owned_stream = other.owned_stream;
