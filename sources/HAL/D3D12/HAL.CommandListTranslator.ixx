...
texture_layout get_texture_layout(const Resource* resource, UINT sub_resource, ivec3 box)
{
	...
	return { size , rows_count , res_stride , res_stride * rows_count,D3D12_TEXTURE_DATA_PITCH_ALIGNMENT, from_native(Layouts.Footprint.Format) };
			...
			void transitions(HAL::Barriers& barriers)
{
				auto& native_transitions = barriers.get_native();
				if (!native_transitions.empty())
				{

					compiler.ResourceBarrier((UINT)native_transitions.size(), native_transitions.data());
				}
}
		};
