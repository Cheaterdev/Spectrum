...
namespace HAL
{
    namespace API
    {
        ...
        void CommandList::begin(HAL::CommandAllocator& allocator)
        {
            if (!m_commandList)
            {
                TEST(*m_device, m_device->get_native_device()->CreateCommandList(0, to_native(type), allocator.m_commandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_commandList)));
            }
            else
            {
                TEST(*m_device, m_commandList->Reset(allocator.m_commandAllocator.Get(), nullptr));
            }
