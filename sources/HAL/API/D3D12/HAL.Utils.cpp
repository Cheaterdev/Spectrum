...
D3D12_PRIMITIVE_TOPOLOGY_TYPE to_native(PrimitiveTopologyType topology)
{
...
D3D12_SAMPLER_DESC to_native(const SamplerDesc& desc)
{
...
D3D12_COMMAND_LIST_TYPE to_native(CommandListType type)
{
...
D3D12_HEAP_TYPE to_native(HeapType type)
{
...
HeapType from_native(D3D12_HEAP_TYPE type)
{
...
D3D12_HEAP_FLAGS to_native(HeapFlags flags)
{
...
D3D12_DESCRIPTOR_HEAP_FLAGS to_native(DescriptorHeapFlags flags)
{
...
D3D12_DESCRIPTOR_HEAP_TYPE to_native(DescriptorHeapType type)
{
...
D3D12_CULL_MODE to_native(CullMode mode)
{
...
D3D12_FILL_MODE to_native(FillMode mode)
{
...
D3D12_BLEND to_native(Blend blend)
{
...
D3D12_STENCIL_OP to_native(StencilOp mode)
{
...
D3D12_DEPTH_STENCILOP_DESC to_native(StencilDesc desc)
{
...
D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS to_native(RaytracingBuildFlags flags)
{
...
DXGI_FORMAT to_native(Format format)
{
...
Format from_native(DXGI_FORMAT format)
{
    ...
    if (format == DXGI_FORMAT_SAMPLER_FEEDBACK_MIN_MIP_OPAQUE)  return Format::UNKNOWN;

    ASSERT(false);
    return Format::UNKNOWN;
}

D3D12_DESCRIPTOR_RANGE_TYPE to_native(DescriptorRange range)
{
...
D3D12_SHADER_VISIBILITY to_native(ShaderVisibility visibility)
{
...
D3D12_BARRIER_LAYOUT to_native(TextureLayout layout)
{
...
D3D12_BARRIER_ACCESS to_native(BarrierAccess flags)
{
...
D3D12_RESOURCE_FLAGS to_native(const ResFlags& flags)
{
...
ResFlags from_native(D3D12_RESOURCE_FLAGS flags)
{
...
D3D12_VIEWPORT to_native(Viewport v)
{
    return D3D12_VIEWPORT{ v.pos.x, v.pos.y, v.size.x, v.size.y, v.depths.x, v.depths.y };
}

D3D_PRIMITIVE_TOPOLOGY to_native(HAL::PrimitiveTopologyType topology, HAL::PrimitiveTopologyFeed feedType, bool adjusted, uint controlpoints)
{
    switch (topology)
    {
...
D3D12_HIT_GROUP_TYPE to_native(HitGroupType type)
{
...
D3D12_RAYTRACING_GEOMETRY_TYPE to_native(GeometryType type)
{
...
D3D12_RAYTRACING_GEOMETRY_FLAGS to_native(GeometryFlags flags)
{
