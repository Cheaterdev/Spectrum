// ============================================================================
// THIS FILE IS AUTOMATICALLY GENERATED - DO NOT MANUALLY EDIT
// ============================================================================
// Generated by Prism from .prism files in sources/Prism/defs/
// Changes will be lost on next generation. Edit the .prism source files instead.
// ============================================================================
export module HAL:Autogen.PSO.Dev.DebugViewMesh;

import Core;
import :PSO;
import :Enums;
import :Types;


export namespace PSOS
{
	namespace Dev
	{
		struct DebugViewMesh: public PSOBase
		{
			struct Keys {
				GEN_DEF_COMP(Keys);
			private:
				SERIALIZE()
				{
				}
			};

			GEN_GRAPHICS_PSO(DebugViewMesh)


			SimplePSO init_pso(Keys & key, std::function<void(SimplePSO&, Keys&)> f)
			{


				SimplePSO mpso("DebugViewMesh");
				if(f) f(mpso,key);

				mpso.root_signature = Layouts::DefaultLayout;

				mpso.mesh.file_name = "shaders/debug/debug_view_mesh_shader.hlsl";
				mpso.mesh.entry_point = "VS";
				mpso.mesh.flags = HAL::ShaderOptions::None;
			
				mpso.amplification.file_name = "shaders/debug/debug_view_mesh_shader.hlsl";
				mpso.amplification.entry_point = "AS";
				mpso.amplification.flags = HAL::ShaderOptions::None;
			
				mpso.pixel.file_name = "shaders/debug/debug_view_mesh.hlsl";
				mpso.pixel.entry_point = "PS";
				mpso.pixel.flags = HAL::ShaderOptions::None;
			

				mpso.rtv_formats = { HAL::Format::R8G8B8A8_UNORM };	
				mpso.blend = {  };

				mpso.ds =HAL::Format::D32_FLOAT;
				mpso.cull =HAL::CullMode::None;
				return mpso;
			}

			private:
			SERIALIZE()
			{
				ar&NVP(wrap(psos));
			}
		};
	}
}