// ============================================================================
// THIS FILE IS AUTOMATICALLY GENERATED - DO NOT MANUALLY EDIT
// ============================================================================
// Generated by Prism from .prism files in sources/Prism/defs/
// Changes will be lost on next generation. Edit the .prism source files instead.
// ============================================================================
export module HAL:Autogen.Tables.Post.Upscale.FSR;

import Core;
import :SIG;
import :Types;
import :HLSL;
import :Enums;
import :Autogen.Tables.Post.Upscale.FSRConstants;
export namespace Table
{
	#pragma pack(push, 1)
	namespace Post
	{
		namespace Upscale
		{
			struct FSR
			{
				static constexpr SlotID ID = SlotID::FSR;
				HLSL::Texture2D<float4> source;
				HLSL::RWTexture2D<float4> target;
				Table::Post::Upscale::FSRConstants constants;
				HLSL::Texture2D<float4>& GetSource() { return source; }
				HLSL::RWTexture2D<float4>& GetTarget() { return target; }
				Table::Post::Upscale::FSRConstants& GetConstants() { return constants; }
				static constexpr SIG_TYPE TYPE = SIG_TYPE::Table;
				template<class Compiler>
				void compile(Compiler& compiler) const
				{
					compiler.compile(source, "FSR::source");
					compiler.compile(target, "FSR::target");
					compiler.compile(constants, "FSR::constants");
				}
				struct Compiled
				{
					uint source; // Texture2D<float4>
					uint target; // RWTexture2D<float4>
					Table::Post::Upscale::FSRConstants::Compiled constants; // FSRConstants

			
					private:
					SERIALIZE()
					{
						ar& NVP(constants);
					}


				};

				static std::string get_typename()
				{
					return "Tables::FSR";
				}
				private:
				SERIALIZE()
				{
					ar& NVP(constants);
				}

			};
		}
	}
	#pragma pack(pop)
}

