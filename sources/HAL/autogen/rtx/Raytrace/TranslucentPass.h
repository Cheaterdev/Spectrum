// ============================================================================
// THIS FILE IS AUTOMATICALLY GENERATED - DO NOT MANUALLY EDIT
// ============================================================================
// Generated by Prism from .prism files in sources/Prism/defs/
// Changes will be lost on next generation. Edit the .prism source files instead.
// ============================================================================
#pragma once
namespace Raytrace
{

	struct TranslucentPass: public RaytracePass<TranslucentPass>
	{
		using Payload = Table::Raytrace::TranslucentPayload;

		using LocalData =  Slots::Meshes::MaterialInfo;


		static const constexpr uint ID = 3;

		static const constexpr std::string_view shader = "shaders\\rtx/translucency.hlsl";
		static const constexpr std::wstring_view name = L"TranslucentPass_GROUP";
		static const constexpr std::wstring_view hit_name = L"TranslucentClosestHitShader";
		static const constexpr std::wstring_view miss_name = L"TranslucentMissShader";
		// Empty when this pass doesn't declare an any_hit stage -- always emitted
		// (rather than conditionally, per pass) so RaytracePass<Desc>'s init()/
		// init_for_material() can check Desc::any_hit_name.empty() uniformly
		// instead of needing per-Desc SFINAE for a member that may not exist.
		static const constexpr std::wstring_view any_hit_name = L"";

		static const constexpr bool per_material = true;
	};
}