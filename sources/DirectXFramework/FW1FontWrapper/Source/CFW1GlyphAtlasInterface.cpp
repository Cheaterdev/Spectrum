...
// Set sheet shader resources
    HRESULT STDMETHODCALLTYPE CFW1GlyphAtlas::BindSheet(Render::CommandList::ptr& pContext, UINT SheetIndex, UINT Flags)
    {
        if (SheetIndex < m_sheetCount)
...
// Flush all sheets with possible new glyphs
    void STDMETHODCALLTYPE CFW1GlyphAtlas::Flush(Render::CommandList::ptr& pContext)
    {
        UINT first = 0;
        UINT end = 0;
        EnterCriticalSection(&m_glyphSheetsCriticalSection);
