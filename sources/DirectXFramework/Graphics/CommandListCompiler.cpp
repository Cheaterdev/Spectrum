...
namespace Graphics
{
	...
	void CommandListCompiler::create(CommandListType type)
	{
		...
		TEST(Device::get(), Device::get().get_native_device()->CreateCommandAllocator(t, IID_PPV_ARGS(&compiled.m_commandAllocator)));
		TEST(Device::get(), Device::get().get_native_device()->CreateCommandList(0, t, compiled.m_commandAllocator.Get(), nullptr, IID_PPV_ARGS(&compiled.m_commandList)));
	...
	void CommandListCompiler::reset()
	{
		...
		TEST(Device::get(), compiled.m_commandList->Reset(compiled.m_commandAllocator.Get(), nullptr));
	}
