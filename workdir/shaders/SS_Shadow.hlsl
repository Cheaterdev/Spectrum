#pragma once
#include "autogen/DispatchParameters.h"
...
//		
		#define HARD_SHADOW_SAMPLES 60				// Number of initial shadow samples that will produce a hard shadow, and not perform sample-averaging.
...
#define READ_COUNT (SAMPLE_COUNT / WAVE_SIZE + 2)
    ...
    Result WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
	{
		...
		[unroll] for (i = 0; i < READ_COUNT; i++)
		{
			...
			depths.x = inParameters.GetDepthTexture().SampleLevel(pointBorderSampler, read_xy * inParameters.InvDepthTextureSize, 0);
			depths.y = inParameters.GetDepthTexture().SampleLevel(pointBorderSampler, (read_xy + offset_xy) * inParameters.InvDepthTextureSize, 0);
#else
			int bias = bilinear > 0 ? 1 : -1;
			int2 offset_xy = int2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);

            depths.x = inParameters.GetDepthTexture().SampleLevel(pointBorderSampler, read_xy * inParameters.InvDepthTextureSize, 0);
            depths.y = inParameters.GetDepthTexture().SampleLevel(pointBorderSampler, read_xy * inParameters.InvDepthTextureSize, 0, offset_xy);
#endif
