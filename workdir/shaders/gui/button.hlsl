...
float4 PS(quad_output i) : SV_TARGET0
{
    ...
    float alpha = 1 - smoothstep(0.6, 0.7, len);
    ...
    float shadow2 = 1 - (1 - min(shadow_mask.x, shadow_mask.y)) * color1.a * smoothstep(0.4, 0.8, len);

    return float4(shadow2.x * result, alpha + (1 - 0.5f * color1.a) * shadow);

}
#endif


#ifdef BUILD_FUNC_PS_Flat
cbuffer cbParams : register(b0)
{
    float4 color1;
    float4 color2;
...
float4 PS_Flat(quad_output i) : SV_TARGET0
{
    ...
    float t = 0.1 / scale;
    float alpha = 1 - smoothstep(0.6 - t, 0.6 + t, len);


    return float4(color1.xyz, color1.a * alpha);
