...
// comment for the shim pattern). See sig_reblur_classifytiles.hlsl's comment
// for why NRD_INTERNAL/NRD_SIGNAL/NRD_MODE must be defined here.
#define NRD_INTERNAL
#define NRD_SIGNAL DIFF
#define NRD_MODE RADIANCE

#include "../autogen/REBLUR_TemporalAccumulationResources.h"

#define NRD_CONSTANTS_START( resourceName ) \
	static const ConstantBuffer<REBLUR_TemporalAccumulationResources> _nrd_res = GetREBLUR_TemporalAccumulationResources(); \
...
#define NRD_INPUTS_END \
	...
	static const Texture2D<float3> gIn_Mv = _nrd_res.GetGIn_Mv(); \
	static const Texture2D<float> gPrev_ViewZ = _nrd_res.GetGPrev_ViewZ(); \
	static const Texture2D<float4> gPrev_Normal_Roughness = _nrd_res.GetGPrev_Normal_Roughness(); \
	static const Texture2D<uint> gPrev_InternalData = _nrd_res.GetGPrev_InternalData(); \
	static const Texture2D<float> gIn_DisocclusionThresholdMix = _nrd_res.GetGIn_DisocclusionThresholdMix(); \
	static const Texture2D<float> gIn_DiffConfidence = _nrd_res.GetGIn_DiffConfidence(); \
	static const Texture2D<float4> gIn_Diff = _nrd_res.GetGIn_Diff(); \
	static const Texture2D<float4> gHistory_Diff = _nrd_res.GetGHistory_Diff(); \
	static const Texture2D<float> gHistory_DiffFast = _nrd_res.GetGHistory_DiffFast();

#define NRD_OUTPUTS_START
#define NRD_OUTPUT( resourceType, dataType, resourceName, regName, bindingIndex )
#define NRD_OUTPUTS_END \
	static const RWTexture2D<float2> gOut_Data1 = _nrd_res.GetGOut_Data1(); \
