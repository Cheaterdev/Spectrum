...
// comment for the shim pattern). REBLUR_Validation.resources.hlsli's
// resource list doesn't depend on NRD_DIFF/NRD_SPEC/NRD_MODE (fixed layout
// regardless), but NRD_INTERNAL is still defined for consistency with the
// rest of REBLUR's vendored source, which does use these elsewhere.
#define NRD_INTERNAL
#define NRD_SIGNAL DIFF
#define NRD_MODE RADIANCE

#include "../autogen/REBLUR_ValidationResources.h"

#define NRD_CONSTANTS_START( resourceName ) \
	static const ConstantBuffer<REBLUR_ValidationResources> _nrd_res = GetREBLUR_ValidationResources(); \
...
#define NRD_CONSTANTS_END \
	...
	static const uint gHasDiffuse = _nrd_res.GetGHasDiffuse(); \
	static const uint gHasSpecular = _nrd_res.GetGHasSpecular();
...
#define NRD_INPUTS_END \
	...
	static const Texture2D<uint> gIn_Data2 = _nrd_res.GetGIn_Data2(); \
