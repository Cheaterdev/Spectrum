// Item 7/8 SIG port (see [[project-nrd-integration]] and sig_clear.hlsl's
// comment for the general shim pattern). Constants route through the real
// SIGMASharedConstants CBV now (Item 8 -- see HAL.NRD.cpp's
// fill_sigma_shared_constants), same pattern REBLUR's own shims use
// (sig_reblur_classifytiles.hlsl).
#include "../autogen/SIGMA_ClassifyTilesResources.h"

#define NRD_CONSTANTS_START( resourceName ) \
	static const ConstantBuffer<SIGMA_ClassifyTilesResources> _nrd_res = GetSIGMA_ClassifyTilesResources(); \
