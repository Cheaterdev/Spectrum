...
// comment for the shim pattern). NRD_INTERNAL must be defined before
// NRD.hlsli is reached (via the #include below) -- it gates the
// RADIANCE/SH/OCCLUSION/DIFF/SPEC/NRD_DIFF/NRD_SPEC macros REBLUR's own
// resources.hlsli #if blocks depend on; without it those macros are
// undefined (treated as 0 in #if), silently taking the wrong branch instead
// of failing loudly. NRD_SIGNAL=DIFF / NRD_MODE=RADIANCE match this
// instance's actual permutation (REBLUR_DIFFUSE, non-SH, non-occlusion).
#define NRD_INTERNAL
#define NRD_SIGNAL DIFF
#define NRD_MODE RADIANCE

#include "../autogen/REBLUR_ClassifyTilesResources.h"

#define NRD_CONSTANTS_START( resourceName ) \
	static const ConstantBuffer<REBLUR_ClassifyTilesResources> _nrd_res = GetREBLUR_ClassifyTilesResources(); \
...
#define NRD_OUTPUTS_END \
	static const RWTexture2D<float> gOut_Tiles = _nrd_res.GetGOut_Tiles();
