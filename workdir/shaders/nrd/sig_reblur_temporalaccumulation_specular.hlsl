...
#include "../autogen/REBLUR_TemporalAccumulationSpecularResources.h"

#define NRD_CONSTANTS_START( resourceName ) \
	static const ConstantBuffer<REBLUR_TemporalAccumulationSpecularResources> _nrd_res = GetREBLUR_TemporalAccumulationSpecularResources(); \
...
#define NRD_INPUTS_END \
	...
	static const Texture2D<float> gIn_SpecConfidence = _nrd_res.GetGIn_SpecConfidence(); \
	static const Texture2D<float4> gIn_Spec = _nrd_res.GetGIn_Spec(); \
	static const Texture2D<float4> gHistory_Spec = _nrd_res.GetGHistory_Spec(); \
	static const Texture2D<float> gHistory_SpecFast = _nrd_res.GetGHistory_SpecFast(); \
	static const Texture2D<float> gPrev_SpecHitDistForTracking = _nrd_res.GetGPrev_SpecHitDistForTracking(); \
