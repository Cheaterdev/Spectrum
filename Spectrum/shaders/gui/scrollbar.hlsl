...
float4 PS(quad_output i) : SV_TARGET0
{
    ...
    float alpha = 1 - smoothstep(0.5, 0.9, len);
	...
	float back = color1.a*0.2+0.8;
	float shadow2 = 1 - (1 - min(shadow_mask.x, shadow_mask.y)) *back * smoothstep(0.4, 0.7, len);



	//float front = 
	return float4(shadow2.x * result, alpha + (1 - back) * shadow);
