...
RWTexture2D<float4> light : register(u1);
Texture2D<float4> tex_albedo : register(t0,space1);
Texture2D<float4> tex_normal : register(t1,space1);
TextureCube<float4> tex_cube : register(t2,space1);
#endif
...
void set_result(int2 tc, float ao)
{
float4 albedo = tex_albedo[tc];
float4 normal = tex_normal[tc]*2-1;
float4 cube_color = tex_cube.SampleLevel(LinearSampler,normal.xyz,8)*0.1;
light[tc]+= albedo*cube_color*pow(ao,4);//*dot(normal.xyz,camera.direction);
}
...
void main(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
	...
	float4 res;
	res.x = BilateralUpsample(HiDepths.x, HiSSAOs.x, LoDepths.xyzw, LoSSAOs.xyzw);
	 res.y = BilateralUpsample(HiDepths.y, HiSSAOs.y, LoDepths.yzwx, LoSSAOs.yzwx);
  res.z = BilateralUpsample(HiDepths.z, HiSSAOs.z, LoDepths.zwxy, LoSSAOs.zwxy);
   res.w =BilateralUpsample(HiDepths.w, HiSSAOs.w, LoDepths.wxyz, LoSSAOs.wxyz);
	
	

    AoResult[OutST + int2(-1,  0)] = res.x;
    AoResult[OutST + int2(0,  0)] = res.y;
    AoResult[OutST + int2(0, -1)] = res.z;
    AoResult[OutST + int2(-1, -1)] = res.w;
		#ifdef  COMBINE_RESULT

		set_result(OutST + int2(-1,  0),res.x);
		set_result(OutST + int2(0,  0),res.y);
		set_result(OutST + int2(0,-1),res.z);
		set_result(OutST + int2(-1,  -1),res.w);
